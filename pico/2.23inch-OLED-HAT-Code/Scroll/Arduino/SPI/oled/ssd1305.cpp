...
{
  command(SSD1305_DEACTIVATE_SCROLL);
  if(HORIZONTAL) //
   { 
      command(SSD1305_LEFT_HORIZONTAL_SCROLL);
      command(0x01);//Set number of column scroll offset
      command(0x00);//Define start page address 
      command(0x00);//Set time interval between each scroll step in terms of frame frequency
      command(0x07); //Define end page address 
   }
   else if(VERTICAL) //
   {
      command(SSD1305_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL);
      command(0x00);//Set number of column scroll offset 
      command(0x00);//Define start page address
      command(0x00);//Set time interval between each scroll step in terms of frame frequency
      command(0x07);//Define end page address 
...
{
  command(SSD1305_ACTIVATE_SCROLL);  
}
