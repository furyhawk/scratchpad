...
class i2c_equipment : public SensorPCF85063
{
private:
	//SensorPCF85063 rtc;
	RtcDateTime_t time;
...
public:
	...
	void set_rtcAlarmSec(int sec); //只关注间隔时间,不注重具体的时间
	void set_rtcAlarmMinute(int min);
  	void set_rtcAlarmHour(int hour);
	void set_rtcAlarmDay(int day);
	
	RtcDateTime_t get_rtcTime();
	EventGroupHandle_t get_rtcGroupHandle();
};
