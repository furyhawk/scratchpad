...
esp_err_t i2c_init() {
    ESP_LOGI(TAG, "Initializing I2C: SDA=%d, SCL=%d, Freq=%dHz", 
...
int pmu_register_read(uint8_t devAddr, uint8_t regAddr, uint8_t *data, uint8_t len) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "PMU READ FAILED! Register: 0x%02X, Error: %s", regAddr, esp_err_to_name(ret));
        return -1;
    }
...
int pmu_register_write_byte(uint8_t devAddr, uint8_t regAddr, uint8_t *data, uint8_t len) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "PMU WRITE FAILED! Register: 0x%02X, Error: %s", regAddr, esp_err_to_name(ret));
        return -1;
    }
